...
module Data.Interface.Module.Interface where
...
findExport iface en = case en of
    ForeignName (Qual m n) ->
        Just $ Named (rawName n) $ ReExport m (namespace n)
    LocalName n -> case namespace n of
        Values -> fmap LocalValue <$> lookupRawName n (moduleValueDecls iface)
        Types -> fmap LocalType <$> lookupRawName n (moduleTypeDecls iface)
